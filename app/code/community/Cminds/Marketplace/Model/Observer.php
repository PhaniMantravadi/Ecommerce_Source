...
class Cminds_Marketplace_Model_Observer extends Mage_Core_Model_Abstract
{
    ...
    public function setTaxInfo($observer)
    {
        try {
            $quoteItems = $observer->getEvent()->getCart()->getQuote()->getAllItems();
            foreach ($quoteItems as $quoteItem) {
                $productInfo = Mage::getModel('catalog/product')->load($quoteItem->getProduct()->getId());
				
                $quoteItem->setData('seller_tax_type',$productInfo->getSellerTaxType())
                    ->setData('seller_tax_rate', str_replace('%', '',$productInfo->getSellerTaxRate()))
                    ->setData('seller_id', $productInfo->getCreatorId())
                    ->save();
