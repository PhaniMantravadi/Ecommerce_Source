       ...
       $supplierLogo = Mage::helper('marketplace')->getSupplierLogo($customer->getId());
        $fieldset->addField(
            ...
            array(
                ...
                'name'      => 'supplier_logo',
                'after_element_html' => ($supplierLogo) ? '<br>Uploaded Logo : <img height="50px" width="50px" src="'.$supplierLogo.'" alt=""/>' : '',
        ...
        $fieldset = $form->addFieldset(
            'supplier_invoice_info',
            array(
                'legend' => Mage::helper('marketplace')->__('Supplier Invoice Info'),
            )
        );
        $fieldset->addField(
            ...
            'text',
            array(
                'label'    => Mage::helper('marketplace')->__('Supplier Invoice Auth Label'),
                'name'     => 'supplier_invoice_auth_label',
                'value'     => $customer->getData('supplier_invoice_auth_label')
            )
        );
        $supplierInvoiceLogo = Mage::helper('marketplace')->getSupplierInvoiceHeaderLogo($customer->getId());
        $fieldset->addField(
            'supplier_invoice_logo',
            'file',
            array(
                'label'     => Mage::helper('marketplace')->__('Supplier Invoice Logo'),
                'name'      => 'supplier_invoice_logo',
                'after_element_html' => ($supplierInvoiceLogo) ? '<br>Uploaded Sign : <img height="50px" width="50px" src="'.$supplierInvoiceLogo.'" alt=""/>' : '',
                'value'     => $customer->getData('supplier_invoice_logo'),
                'wysiwyg'   => true,
            )
        );

        $supplierInvoiceAuthSign = Mage::helper('marketplace')->getSupplierInvoiceSign($customer->getId());
        $fieldset->addField(
            'supplier_invoice_auth_sign',
            'file',
            array(
                'label'     => Mage::helper('marketplace')->__('Supplier Invoice Auth Sign'),
                'name'      => 'supplier_invoice_auth_sign',
                'after_element_html' => ($supplierInvoiceAuthSign) ? '<br>Uploaded Sign : <img height="50px" width="50px" src="'.$supplierInvoiceAuthSign.'" alt=""/>' : '',
                'value'     => $customer->getData('supplier_invoice_auth_sign'),
                'wysiwyg'   => true,
            )
        );
