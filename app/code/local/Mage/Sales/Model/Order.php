...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function canInvoice()
    {
        ...
        $state = $this->getState();
        if ($this->isCanceled() || $state === self::STATE_COMPLETE || $state === self::STATE_CLOSED ||  ($this->getStatus() == self::STATE_PENDING_PAYMENT) ||
            ($this->getStatus() == 'payment_failed') || !$this->canAdminShipOrInvoice()) {
            return false;
        }
    ...
    public function canShip()
    {
        ...
        if ($this->getIsVirtual() || $this->isCanceled() || ($this->getStatus() == self::STATE_PENDING_PAYMENT) ||
            ($this->getStatus() == 'payment_failed') || !$this->canAdminShipOrInvoice()) {
            return false;
        }
...
/*********************** STATUSES ***************************/
    ...
    public function canAdminShipOrInvoice() {
        try {
            $marketPlaceHelper = Mage::helper('marketplace');
            return $marketPlaceHelper->adminCanShipInvoice($this);
        } catch (Exception $e) {
            Mage::logException($e);
        }
    }
