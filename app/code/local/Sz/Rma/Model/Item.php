 ...
 */
class Sz_Rma_Model_Item extends Mage_Core_Model_Abstract
{
    /**
     * Entity code.
    ...
    {
        return Mage::getModel('sz_rma/item_attribute_source_state_status')->getAllOptions();
    }
     ...
     */
    public function setRma(Sz_Rma_Model_Rma $rma)
    {
        $this->_rma = $rma;
    ...
    public function getStatusLabel()
    {
        if (is_null(parent::getStatusLabel())){
            $this->setStatusLabel(
                Mage::getModel('sz_rma/item_attribute_source_status')->getItemLabel($this->getStatus())
            );
        }
    ...
    {
        $qtyReturnedChange = 0;
        if ($this->getOrigData('status') == Sz_Rma_Model_Rma_Source_Status::STATE_APPROVED) {
            if ($this->getStatus() == Sz_Rma_Model_Rma_Source_Status::STATE_APPROVED) {
     ...
     * @return  Sz_Rma_Model_Item_Attribute | null
     */
