...
class Sz_Rma_Model_Resource_Item extends Mage_Eav_Model_Entity_Abstract
{
    /**
    ...
    public function getAuthorizedItems($rmaId)
    {
        ...
        $select = $adapter->select()
            ->from($this->getTable('sz_rma/item_entity'), array())
            ->where('rma_entity_id = ?', $rmaId)
            ->where('status = ?', Sz_Rma_Model_Rma_Source_Status::STATE_AUTHORIZED)
    ...
    public function getItemsIdsByOrder($orderId)
    {
        ...
        $subSelect = $adapter->select()
            ->from(
                array('main' => $this->getTable('sz_rma/rma')),
                array()
            )
        ...
        $select = $adapter->select()
            ->from(
                array('item_entity' => $this->getTable('sz_rma/item_entity')),
