 ...
 */
class Sz_Rma_Model_Rma extends Mage_Core_Model_Abstract
{
    /**
     * XML configuration paths
     */
    const XML_PATH_SECTION_RMA       = 'sales/sz_rma/';
    const XML_PATH_ENABLED           = 'sales/sz_rma/enabled';
    const XML_PATH_USE_STORE_ADDRESS = 'sales/sz_rma/use_store_address';
     /**
     * Rma Instance
     *
     * @var Sz_Rma_Model_Rma
     */
    protected $_rma = null;
    ...
    protected function _afterSave()
    {
        ...
        /** @var $gridModel Sz_Rma_Model_Grid */
        $gridModel = Mage::getModel('sz_rma/grid');
        $gridModel->addData($this->getData());
        $gridModel->save();

        Mage::getModel('sz_rma/rma_status_history')->setRma($this)->saveSystemComment();
    ...
    {
        return Mage::getModel('sz_rma/rma_source_status')->getAllOptionsForGrid();
    }
     ...
     *
     * @return Sz_Rma_Model_Resource_Item_Collection
     */
    ...
    public function canClose()
    {
        ...
        if ($status === Sz_Rma_Model_Rma_Source_Status::STATE_CLOSED
            || $status === Sz_Rma_Model_Rma_Source_Status::STATE_PROCESSED_CLOSED) {
            return false;
        }
    ...
    {
        if ($this->canClose()) {
            $this->setStatus(Sz_Rma_Model_Rma_Source_Status::STATE_CLOSED);
        }
        return $this;
    }
    ...
    public function _sendRmaEmailWithItems($rootConfig)
    {
        ...
        $returnAddress = Mage::helper('sz_rma')->getReturnAddress('html', array(), $this->getStoreId());
    ...
    protected function _preparePost($item)
    {
        ...
        $stat = Sz_Rma_Model_Item_Attribute_Source_Status::STATE_PENDING;
        if (!empty($preparePost['status'])) {
            $status = Mage::getModel('sz_rma/item_attribute_source_status');
        ...
        $preparePost['product_admin_name']  = Mage::helper('sz_rma')->getAdminProductName($realItem);
        $preparePost['product_admin_sku']   = Mage::helper('sz_rma')->getAdminProductSku($realItem);
        ...
        if ($errors) {
            $session = Mage::getSingleton('core/session');
            $session->addError(
                Mage::helper('sz_rma')->__('There is an error in quantities for item %s.', $preparePost['product_name'])
            );
        }
     ...
     *
     * @param  Sz_Rma_Model_Item $itemModels
     * @param  $orderId
     * @return array|bool
     */
    protected function _checkPost($itemModels, $orderId)
    {
        ...
        $errorKeys  = array();
        if (!$this->getIsUpdate()) {
            $availableItems = Mage::helper('sz_rma')->getOrderItems($orderId);
        } else {
            $availableItems = Mage::getResourceModel('sz_rma/item')->getOrderItemsCollection($orderId);
        }
        ...
        foreach ($itemModels as $item) {
            ...
            if ($this->getIsUpdate()) {
                ...
                $validation['dummy'] = -1;
                $previousValue = null;
                $escapedProductName = Mage::helper('sz_rma')->escapeHtml($item->getProductName());
                foreach ($validation as $key => $value) {
                    if (isset($previousValue) && $value > $previousValue) {
                        $errors[] = Mage::helper('sz_rma')->__('There is an error in quantities for item %s.', $escapedProductName);
                ...
                $qtyToStatus = array(
                    'qty_authorized' => array(
                            'name' => Mage::helper('sz_rma')->__('Authorized Qty'),
                            'status' => Sz_Rma_Model_Rma_Source_Status::STATE_AUTHORIZED
                        ),
                    'qty_returned' => array(
                            'name' => Mage::helper('sz_rma')->__('Returned Qty'),
                            'status' => Sz_Rma_Model_Rma_Source_Status::STATE_RECEIVED
                        ),
                    'qty_approved' => array(
                            'name' => Mage::helper('sz_rma')->__('Approved Qty'),
                            'status' => Sz_Rma_Model_Rma_Source_Status::STATE_APPROVED
                        ),
                ...
                foreach ($qtyToStatus as $qtyKey => $qtyValue) {
                    if ($item->getStatus() === $qtyValue['status']
                        ...
                        $errors[] = Mage::helper('sz_rma')->__('%s for item %s cannot be empty.', $qtyValue['name'], $escapedProductName);
        ...
        foreach ($itemsArray as $key=>$qty) {
            $escapedProductName = Mage::helper('sz_rma')->escapeHtml($availableItemsArray[$key]['name']);
            if (!array_key_exists($key, $availableItemsArray)) {
                $errors[] = Mage::helper('sz_rma')->__('You cannot return %s.', $escapedProductName);
            }
            if (isset($availableItemsArray[$key]) && $availableItemsArray[$key]['qty'] < $qty) {
                $errors[] = Mage::helper('sz_rma')->__('Quantity of %s is greater than you can return.', $escapedProductName);
    ...
    protected function _createItemsCollection()
    {
        ...
        foreach ($data['items'] as $key=>$item) {
            if (isset($item['items'])) {
                ...
                if (($itemModel->getStatus() === Sz_Rma_Model_Item_Attribute_Source_Status::STATE_AUTHORIZED)
    ...
    {
        $label = Mage::helper('sz_rma')->getContactEmailLabel();

        $validator = new Zend_Validate_EmailAddress();
        $validator->setMessage(
            Mage::helper('sz_rma')->__('"%s" invalid type entered.', $label),
            Zend_Validate_EmailAddress::INVALID
        );
        $validator->setMessage(
            Mage::helper('sz_rma')->__('"%s" is not a valid email address.', $label),
            Zend_Validate_EmailAddress::INVALID_FORMAT
        );
        $validator->setMessage(
            Mage::helper('sz_rma')->__('"%s" is not a valid hostname.', $label),
        ...
        $validator->setMessage(
            Mage::helper('sz_rma')->__('"%s" exceeds the allowed length.', $label),
    ...
    public function getShippingMethods($returnItems = false)
    {
        ...
        $rmaItems   = Mage::getResourceModel('sz_rma/item')
    ...
    {
        $shippingDestinationInfo = Mage::helper('sz_rma')->getReturnAddressModel($this->getStoreId());
            ...
            foreach ($shippingRates as $shippingRate) {
                if (
                    in_array(
                        ...
                        array_keys(Mage::helper('sz_rma')->getShippingCarriers())
                    )
                ) {
    ...
    public function getTrackingNumbers()
    {
        if (is_null($this->_trackingNumbers)) {
            $this->_trackingNumbers = Mage::getModel('sz_rma/shipping')
            ->getCollection()
            ->addFieldToFilter('rma_entity_id', $this->getEntityId())
            ->addFieldToFilter('is_admin', array('neq' => Sz_Rma_Model_Shipping::IS_ADMIN_STATUS_ADMIN_LABEL));
        }
    ...
    public function getShippingLabel()
    {
        if (is_null($this->_shippingLabel)) {
            $this->_shippingLabel = Mage::getModel('sz_rma/shipping')
            ->getCollection()
            ->addFieldToFilter('rma_entity_id', $this->getEntityId())
            ->addFieldToFilter('is_admin', Sz_Rma_Model_Shipping::IS_ADMIN_STATUS_ADMIN_LABEL)
            ->getFirstItem();
        }
    ...
    {
        return ($this->getStatus() !== Sz_Rma_Model_Rma_Source_Status::STATE_CLOSED)
            && ($this->getStatus() !== Sz_Rma_Model_Rma_Source_Status::STATE_PROCESSED_CLOSED)
            && ($this->getStatus() !== Sz_Rma_Model_Rma_Source_Status::STATE_PENDING);
    }
    ...
    protected function _isItemsAvailableForPrintLabel()
    {
        ...
        foreach ($collection as $item) {
            if (!in_array($item->getStatus(),
                array(
                    Sz_Rma_Model_Item_Attribute_Source_Status::STATE_AUTHORIZED,
                    Sz_Rma_Model_Item_Attribute_Source_Status::STATE_DENIED,
                ), true)
            ) {
                return false;
            }
            if (($item->getStatus() === Sz_Rma_Model_Item_Attribute_Source_Status::STATE_AUTHORIZED)
    ...
    {
        return (bool)(
            Mage::getModel('sz_rma/rma_source_status')->getButtonDisabledStatus($this->getStatus())
    ...
    public function _isItemsNotInPendingStatus()
    {
        ...
        foreach ($collection as $item) {
            if ($item->getStatus() == Sz_Rma_Model_Item_Attribute_Source_Status::STATE_PENDING) {
                return false;
            }
        }
        return true;
