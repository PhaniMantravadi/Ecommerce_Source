 ...
 */
class Sz_Rma_Model_Pdf_Rma extends Mage_Sales_Model_Order_Pdf_Abstract
{
    /**
    ...
    public function getPdf($rmaArray = array())
    {
        ...
        if (!(is_array($rmaArray) && (count($rmaArray) == 1))){
            Mage::throwException(Mage::helper('sz_rma')->__('Only one RMA is available for printing'));
        }
        $rma = $rmaArray[0];
        ...
        $page->drawText(
            Mage::helper('sz_rma')->__('Return # ') . $rma->getIncrementId() . ' - ' . $rma->getStatusLabel(),
            35,
            $this->y - 40,
            'UTF-8'
        );
        ...
        $page->drawText(
            Mage::helper('sz_rma')->__('Return Date: ') .
        ...
        $page->drawText(
            Mage::helper('sz_rma')->__('Order # ') . $rma->getOrder()->getIncrementId(),
            35,
            $this->y - 60,
            'UTF-8'
        );
        ...
        $page->drawText(
            Mage::helper('sz_rma')->__('Order Date: ') .
        ...
        $returnAddress = $this
            ->_formatAddress(Mage::helper('sz_rma')
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
        $page->drawText(Mage::helper('sz_rma')->__('Shipping Address:'), 35, $this->y - 10, 'UTF-8');

        $page->drawText(Mage::helper('sz_rma')->__('Return Address:'), 315, $this->y - 10, 'UTF-8');
    ...
    protected function _addItemTableHead($page)
    {
        ...
        $page->drawText(
            Mage::helper('sz_rma')->__('Product Name'),
            $this->getProductNameX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('sz_rma')->__('SKU'),
            $this->getProductSkuX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('sz_rma')->__('Condition'),
            $this->getConditionX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('sz_rma')->__('Resolution'),
            $this->getResolutionX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('sz_rma')->__('Requested Qty'),
            $this->getQtyRequestedX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('sz_rma')->__('Qty'),
            $this->getQtyX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('sz_rma')->__('Status'),
            $this->getStatusX(),
            $this->y,
            'UTF-8'
        );
    ...
    protected function _drawRmaItem($item, $page)
    {
        ...
        $page->drawText(
            Mage::helper('sz_rma')->parseQuantity($item->getQtyRequested(), $item),
            $this->getQtyRequestedX(),
            $this->y,
            'UTF-8'
        );
        ...
        $page->drawText(
            Mage::helper('sz_rma')->getQty($item),
            $this->getQtyX(),
            $this->y,
            'UTF-8'
        );
