 ...
 */
class Sz_Rma_Helper_Data extends Mage_Core_Helper_Abstract
{
    /**
    ...
    {
        return Mage::getStoreConfigFlag(Sz_Rma_Model_Rma::XML_PATH_ENABLED);
    }
    ...
    public function getOrderItems($orderId, $onlyParents = false)
    {
        ...
        if (is_null($this->_orderItems) || !isset($this->_orderItems[$orderId])) {
            $this->_orderItems[$orderId] = Mage::getResourceModel('sz_rma/item')->getOrderItems($orderId);
        }
    ...
    public function getReturnContactName($storeId = null)
    {
        ...
        if (Mage::getStoreConfigFlag(Sz_Rma_Model_Rma::XML_PATH_USE_STORE_ADDRESS, $storeId)) {
            ...
            $name = Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_CONTACT_NAME, $storeId);
    ...
    protected function _getAddressData($store = null)
    {
        ...
        if (Mage::getStoreConfigFlag(Sz_Rma_Model_Rma::XML_PATH_USE_STORE_ADDRESS, $store)) {
            $data = array(
            ...
            $data = array(
                'city'      => Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_CITY, $store),
                'countryId' => Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_COUNTRY_ID, $store),
                'postcode'  => Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_ZIP, $store),
                'region_id' => Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_REGION_ID, $store),
                'street2'   => Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_ADDRESS2, $store),
                'street1'   => Mage::getStoreConfig(Sz_Rma_Model_Shipping::XML_PATH_ADDRESS1, $store)
            );
        }
    ...
    {
        if ($track instanceof Sz_Rma_Model_Rma) {
            return $this->_getTrackingUrl('rma_id', $track);
        } elseif ($track instanceof Sz_Rma_Model_Shipping) {
     ...
     *
     * @param  string $key
     * @param  Sz_Rma_Model_Shipping|Sz_Rma_Model_Rma $model
    ...
    public function canReturnProduct($product, $storeId = null)
    {
        ...
        if (is_null($isReturnable)) {
            $isReturnable = Sz_Rma_Model_Product_Source::ATTRIBUTE_ENABLE_RMA_USE_CONFIG;
        }
        switch ($isReturnable) {
            case Sz_Rma_Model_Product_Source::ATTRIBUTE_ENABLE_RMA_YES:
                return true;
            case Sz_Rma_Model_Product_Source::ATTRIBUTE_ENABLE_RMA_NO:
                return false;
            default: //Use config and NULL
                return Mage::getStoreConfig(Sz_Rma_Model_Product_Source::XML_PATH_PRODUCTS_ALLOWED, $storeId);
        }
    }
    ...
    public function getQty($item)
    {
        ...
        if ($item->getQtyApproved()
            && ($item->getStatus() == Sz_Rma_Model_Rma_Source_Status::STATE_APPROVED)
        ) {
            $qty = $item->getQtyApproved();
        } elseif ($item->getQtyReturned()
            && ($item->getStatus() == Sz_Rma_Model_Rma_Source_Status::STATE_RECEIVED
                || $item->getStatus() == Sz_Rma_Model_Rma_Source_Status::STATE_REJECTED
            )
        ) {
            $qty = $item->getQtyReturned();
        } elseif ($item->getQtyAuthorized()
            && ($item->getStatus() == Sz_Rma_Model_Rma_Source_Status::STATE_AUTHORIZED)
        ) {
