...
class Sz_Rma_Block_Adminhtml_Rma_New_Tab_Items_Order_Grid
    extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        $orderItemsCollection = Mage::getResourceModel('sz_rma/item')->getOrderItemsCollection($orderId);
     ...
     *
     * @return Sz_Rma_Block_Adminhtml_Rma_New_Tab_Items_Order_Grid
     */
    protected function _afterLoadCollection()
    {
        $orderId = Mage::registry('current_order')->getId();
        $itemsInActiveRmaArray = Mage::getResourceModel('sz_rma/item')->getItemsIdsByOrder($orderId);

        $fullItemsCollection = Mage::getResourceModel('sz_rma/item')->getOrderItemsCollection($orderId);
        /**
        ...
        foreach ($fullItemsCollection as $item) {
            ...
            if ($allowed === true) {
                ...
                if (!Mage::helper('sz_rma')->canReturnProduct($product, $item->getStoreId())) {
                    $allowed = false;
                }
            }
        ...
        foreach ($this->getCollection() as $item) {
            ...
            $item->setName(Mage::helper('sz_rma')->getAdminProductName($item));
        }
    ...
    protected function _prepareColumns()
    {
        $this->addColumn('select', array(
            'header'   => Mage::helper('sz_rma')->__('Select'),
            'width'    => '40px',
            'type'     => 'checkbox',
            'align'    => 'center',
            'sortable' => false,
        ...
        $this->addColumn('product_name', array(
            ...
            'renderer' => 'sz_rma/adminhtml_product_bundle_product',
            'index'    => 'name',
            'escape'   => true,
        ));
        ...
        $this->addColumn('price', array(
            'header'=> Mage::helper('sz_rma')->__('Price'),
            'width' => '80px',
            'type'  => 'currency',
            'index' => 'price',
        ));
