 ...
 */
class Sz_Rma_Block_Adminhtml_Rma_Item_Attribute_Edit_Tab_Main
    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
     ...
     *
     * @return Sz_Rma_Block_Adminhtml_Rma_Item_Attribute_Edit_Tab_Main
     */
    protected function _prepareForm()
    {
        ...
        /* @var $helper Sz_Rma_Helper_Eav */
        $helper     = Mage::helper('sz_rma/eav');
        ...
        $element->setLabel(Mage::helper('sz_rma')->__('Input Type'));
        ...
        $fieldset->addField('multiline_count', 'text', array(
            ...
            'label'     => Mage::helper('sz_rma')->__('Lines Count'),
            'title'     => Mage::helper('sz_rma')->__('Lines Count'),
            'required'  => true,
            'class'     => 'validate-digits-range digits-range-2-20',
            'note'      => Mage::helper('sz_rma')->__('Valid range 2-20')
        ), 'frontend_input');

        $fieldset->addField('input_validation', 'select', array(
            'name'      => 'input_validation',
            'label'     => Mage::helper('sz_rma')->__('Input Validation'),
            'title'     => Mage::helper('sz_rma')->__('Input Validation'),
            'values'    => array('' => Mage::helper('sz_rma')->__('None'))
        ), 'default_value_textarea');

        $fieldset->addField('min_text_length', 'text', array(
            'name'      => 'min_text_length',
            'label'     => Mage::helper('sz_rma')->__('Minimum Text Length'),
            'title'     => Mage::helper('sz_rma')->__('Minimum Text Length'),
            'class'     => 'validate-digits',
        ), 'input_validation');

        $fieldset->addField('max_text_length', 'text', array(
            'name'      => 'max_text_length',
            'label'     => Mage::helper('sz_rma')->__('Maximum Text Length'),
            'title'     => Mage::helper('sz_rma')->__('Maximum Text Length'),
            'class'     => 'validate-digits',
        ), 'min_text_length');

        $fieldset->addField('max_file_size', 'text', array(
            'name'      => 'max_file_size',
            'label'     => Mage::helper('sz_rma')->__('Maximum File Size (bytes)'),
            'title'     => Mage::helper('sz_rma')->__('Maximum File Size (bytes)'),
            'class'     => 'validate-digits',
        ), 'max_text_length');

        $fieldset->addField('file_extensions', 'text', array(
            'name'      => 'file_extensions',
            'label'     => Mage::helper('sz_rma')->__('File Extensions'),
            'title'     => Mage::helper('sz_rma')->__('File Extensions'),
            'note'      => Mage::helper('sz_rma')->__('Comma separated'),
        ), 'max_file_size');

        $fieldset->addField('max_image_width', 'text', array(
            'name'      => 'max_image_width',
            'label'     => Mage::helper('sz_rma')->__('Maximum Image Width (px)'),
            'title'     => Mage::helper('sz_rma')->__('Maximum Image Width (px)'),
            'class'     => 'validate-digits',
        ), 'file_extensions');

        $fieldset->addField('max_image_heght', 'text', array(
            'name'      => 'max_image_heght',
            'label'     => Mage::helper('sz_rma')->__('Maximum Image Height (px)'),
            'title'     => Mage::helper('sz_rma')->__('Maximum Image Height (px)'),
            'class'     => 'validate-digits',
        ), 'max_image_width');

        $fieldset->addField('input_filter', 'select', array(
            'name'      => 'input_filter',
            'label'     => Mage::helper('sz_rma')->__('Input/Output Filter'),
            'title'     => Mage::helper('sz_rma')->__('Input/Output Filter'),
            'values'    => array('' => Mage::helper('sz_rma')->__('None')),
        ));
        ...
        $fieldset = $form->addFieldset('front_fieldset', array(
            'legend'    => Mage::helper('sz_rma')->__('Frontend Properties')
        ));

        $fieldset->addField('is_visible', 'select', array(
            'name'      => 'is_visible',
            'label'     => Mage::helper('sz_rma')->__('Show on Frontend'),
            'title'     => Mage::helper('sz_rma')->__('Show on Frontend'),
            'values'    => $yesnoSource,
        ));

        $fieldset->addField('sort_order', 'text', array(
            'name'      => 'sort_order',
            'label'     => Mage::helper('sz_rma')->__('Sort Order'),
            'title'     => Mage::helper('sz_rma')->__('Sort Order'),
            'required'  => true,
            'class'     => 'validate-digits'
        ));
        ...
        $fieldset->addField('used_in_forms', 'multiselect', array(
            'name'         => 'used_in_forms',
            'label'        => Mage::helper('sz_rma')->__('Forms to Use In'),
            'title'        => Mage::helper('sz_rma')->__('Forms to Use In'),
    ...
    {
        return Mage::helper('sz_rma')->__('Properties');
    }
