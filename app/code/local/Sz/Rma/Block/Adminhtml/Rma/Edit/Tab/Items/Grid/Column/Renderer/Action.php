 ...
 */
class Sz_Rma_Block_Adminhtml_Rma_Edit_Tab_Items_Grid_Column_Renderer_Action
    ...
    public function render(Varien_Object $row)
    {
        ...
        foreach ($actions as $action) {
            if (!(isset($action['status_depended'])
                && (($row->getStatus() === Sz_Rma_Model_Rma_Source_Status::STATE_APPROVED)
                    ||($row->getStatus() === Sz_Rma_Model_Rma_Source_Status::STATE_DENIED)
                    ||($row->getStatus() === Sz_Rma_Model_Rma_Source_Status::STATE_REJECTED)))) {
