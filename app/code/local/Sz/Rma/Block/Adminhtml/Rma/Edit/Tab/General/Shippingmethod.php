 ...
 */
class Sz_Rma_Block_Adminhtml_Rma_Edit_Tab_General_Shippingmethod
    extends Sz_Rma_Block_Adminhtml_Rma_Edit_Tab_General_Abstract
{
     ...
     *
     * @var null|Sz_Rma_Model_Rma
     */
    protected $_rma = null;

    public function _construct()
    {
        $buttonStatus       = Sz_Rma_Block_Adminhtml_Rma_Edit_Tab_General_Shippingmethod::PSL_DISALLOWED;
        if ((bool)($this->_getShippingAvailability() && $this->getRma()->isAvailableForPrintLabel())) {
            $buttonStatus   = Sz_Rma_Block_Adminhtml_Rma_Edit_Tab_General_Shippingmethod::PSL_ALLOWED;
        } elseif((bool)$this->getRma()->getButtonDisabledStatus()) {
            $buttonStatus   = Sz_Rma_Block_Adminhtml_Rma_Edit_Tab_General_Shippingmethod::PSL_DISABLED;
        }
    ...
    {
        $carriers = Mage::helper('sz_rma')->getAllowedShippingCarriers($this->getRma()->getStoreId());
    ...
    {
        return Mage::getModel('sz_rma/shipping')
    ...
    public function displayCustomsValue()
    {
        ...
        $recipientAddressCountryCode= Mage::helper('sz_rma')->getReturnAddressModel($storeId)->getCountryId();
    ...
    public function getShowPackagesButton()
    {
        return $this->getLayout()
            ->createBlock('adminhtml/widget_button')
            ->setData(array(
                'label'   => Mage::helper('sz_rma')->__('Show Packages'),
    ...
    public function getPrintButton()
    {
        ...
        return $this->getLayout()
            ->createBlock('adminhtml/widget_button')
            ->setData(array(
                'label'   => Mage::helper('sz_rma')->__('Print'),
                'onclick' => 'setLocation(\'' . $url . '\')'
            ))
            ->toHtml();
    }
    ...
    {
        $carrierCode= $this->getShipment()->getCarrierCode();
        $carrier    = Mage::helper('sz_rma')->getCarrier($carrierCode, $this->getRma()->getStoreId());
        if ($carrier) {
    ...
    {
        $storeId    = $this->getRma()->getStoreId();
        $countryId  = Mage::helper('sz_rma')->getReturnAddressModel($storeId)->getCountryId();
        $carrierCode= $this->getShipment()->getCarrierCode();
        $carrier    = Mage::helper('sz_rma')->getCarrier($carrierCode, $this->getRma()->getStoreId());
        if ($carrier) {
    ...
    public function getContentTypes()
    {
        ...
        $carrierCode= $this->getShipment()->getCarrierCode();
        $carrier    = Mage::helper('sz_rma')->getCarrier($carrierCode, $storeId);
