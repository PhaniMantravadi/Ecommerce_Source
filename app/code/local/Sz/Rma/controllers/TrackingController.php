...
class Sz_Rma_TrackingController extends Mage_Core_Controller_Front_Action
{
    /**
     * Popup action
    ...
    {
        $shippingInfoModel = Mage::getModel('sz_rma/shipping_info')
    ...
    {
        try {
            $data = Mage::helper('sz_rma')->decodeTrackingHash($this->getRequest()->getParam('hash'));
            ...
            $model = Mage::getModel('sz_rma/shipping_info')
                ->loadPackage($this->getRequest()->getParam('hash'));

            $shipping = Mage::getModel('sz_rma/shipping');
