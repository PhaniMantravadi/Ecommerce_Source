...
class Sz_Rma_Adminhtml_Rma_Item_AttributeController extends Mage_Adminhtml_Controller_Action
{
    /**
     ...
     *
     * @return Sz_Rma_Adminhtml_Rma_Item_AttributeController
     */
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                Mage::helper('sz_rma')->__('RMA'),
                Mage::helper('sz_rma')->__('RMA'))
            ->_addBreadcrumb(
                Mage::helper('sz_rma')->__('Manage RMA Item Attributes'),
                Mage::helper('sz_rma')->__('Manage RMA Item Attributes'));
        return $this;
    }
     ...
     *
     * @return Sz_Rma_Model_Item_Attribute
     */
    protected function _initAttribute()
    {
        $attribute = Mage::getModel('sz_rma/item_attribute');
    ...
    {
        /* @var $attributeObject Sz_Rma_Model_Item_Attribute */
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()
                    ->addError(Mage::helper('sz_rma')->__('Attribute is no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
            if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()) {
                $this->_getSession()->addError(Mage::helper('sz_rma')->__('You cannot edit this attribute.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('sz_rma')->__('Edit RMA Item Attribute')
            : Mage::helper('sz_rma')->__('New RMA Item Attribute');
        ...
        if (!$attributeId) {
            ...
            if ($attributeObject->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('sz_rma')->__('Attribute with the same code already exists')
                );
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $data) {
            ...
            try {
                $data = Mage::helper('sz_rma/eav')->filterPostData($data);
            ...
            if ($attributeId) {
                ...
                if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()) {
                    $this->_getSession()->addError(
                        Mage::helper('sz_rma')->__('You cannot edit this attribute.')
                    );
            ...
            try {
                ...
                Mage::dispatchEvent('sz_rma_item_attribute_before_save', array(
                    'attribute' => $attributeObject
                ));

                $attributeObject->save();

//                Mage::dispatchEvent('sz_customer_attribute_save', array(
//                    'attribute' => $attributeObject
//                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('sz_rma')->__('The RMA item attribute has been saved.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('sz_rma')->__('An error occurred while saving the RMA item attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            {
                $this->_getSession()->addError(
                    Mage::helper('sz_rma')->__('You cannot delete this attribute.')
                );
                $this->_redirect('*/*/');
                return;
            }
...
//                Mage::dispatchEvent('sz_customer_attribute_delete', array(
//                    'attribute' => $attributeObject
//                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('sz_rma')->__('The RMA item attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Mage_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('sz_rma')->__('An error occurred while deleting the RMA item attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
