...
class Sz_Rma_Adminhtml_RmaController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Init active menu and set breadcrumb
     *
     * @return Sz_Rma_Adminhtml_RmaController
     */
    ...
    {
        $model = Mage::getModel('sz_rma/rma');
     ...
     * Initialize model
     *
     * @return Sz_Rma_Model_Rma_Create
     */
    protected function _initCreateModel()
    {
        $model = Mage::getModel('sz_rma/rma_create');
        $orderId = $this->getRequest()->getParam('order_id');
    ...
    public function newAction()
    {
        ...
        if (!$orderId) {
            ...
            try {
                ...
                $this->_initModel();
                if (!Mage::helper('sz_rma')->canCreateRma($orderId, true)) {
                    Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('sz_rma')->__('There are no applicable items for return in this order')
                    );
                }
    ...
    public function saveNewAction()
    {
        ...
        if ($data) {
            ...
            try {
                /** @var $model Sz_Rma_Model_Rma */
                $model = $this->_initModel();
                $order = Mage::registry('current_order');
                $rmaData = array(
                    'status'                => Sz_Rma_Model_Rma_Source_Status::STATE_PENDING,
                ...
                if ($result && $result->getId()) {
                    if (isset($data['comment'])
                        ...
                        Mage::getModel('sz_rma/rma_status_history')
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                $rmaItems = Mage::getModel('sz_rma/item')
                    ->getCollection()
                ...
                $model->setStatus(
                    Mage::getModel('sz_rma/rma_source_status')
    ...
    public function addCommentAction()
    {
        try {
            ...
            if (!$rma) {
                Mage::throwException(Mage::helper('sz_rma')->__('Invalid RMA.'));
            }
            ...
            /** @var $history Sz_Rma_Model_Rma_Status_History */
            $history = Mage::getModel('sz_rma/rma_status_history');
    ...
    public function rmaCustomerAction()
    {
        ...
        if ($customerId) {
            $this->getResponse()->setBody(
                $this
                    ->getLayout()
                    ->createBlock('sz_rma/adminhtml_customer_edit_tab_rma')
    ...
    public function rmaOrderAction()
    {
        ...
        $this->getResponse()->setBody(
            $this
                ->getLayout()
                ->createBlock('sz_rma/adminhtml_order_view_tab_rma')
                ->setOrderId($orderId)
                ->toHtml()
        );
    }
    ...
    public function addProductGridAction()
    {
        try {
            ...
            $order = Mage::registry('current_order');
            if (!$order) {
                Mage::throwException(Mage::helper('sz_rma')->__('Invalid order.'));
            }
            $this->loadLayout();
    ...
    public function printAction()
    {
        ...
        if ($rmaId) {
            if ($rma = Mage::getModel('sz_rma/rma')->load($rmaId)) {
                $pdf = Mage::getModel('sz_rma/pdf_rma')->getPdf(array($rma));
    ...
    public function loadAttributesAction()
    {
        ...
        try {
            ...
            $rma_item = Mage::getModel('sz_rma/item');
    ...
    public function loadSplitLineAction()
    {
        ...
        $response = $this
            ->getLayout()
            ->getBlock('sz_rma_edit_items_grid')
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('sales/sz_rma');
    }
    ...
    public function showBundleItemsAction()
    {
        ...
        try {
            if ($orderId && $itemId) {
                /** @var $items Sz_Rma_Model_Resource_Item */
                $items = Mage::getResourceModel('sz_rma/item')->getOrderItems($orderId, $itemId);
                if (empty($items)) {
    ...
    public function showShippingMethodsAction()
    {
        ...
        $this->loadLayout();
        $response = $this->getLayout()
            ->getBlock('sz_rma_shipping_available')
            ->toHtml()
        ;
    ...
    public function pslAction()
    {
        ...
        $this->loadLayout();
        $response = $this->getLayout()
            ->getBlock('sz_rma_shipment_packaging')
            ->toHtml()
        ;
    ...
    protected function _getConfigDataJson($model)
    {
        ...
        $carrier = Mage::helper('sz_rma')->getCarrier($code, $model->getStoreId());
        if ($carrier) {
        ...
        $shippingInformation = $this->getLayout()
            ->createBlock('sz_rma/adminhtml_rma_edit_tab_general_shipping_information')
        ...
        $data = array(
            ...
            'errorQtyOverLimit'         => Mage::helper('sz_rma')->__('The quantity you want to add exceeds the total shipped quantity for some of selected Product(s)'),
            'titleDisabledSaveBtn'      => Mage::helper('sz_rma')->__('Products should be added to package(s)'),
            'validationErrorMsg'        => Mage::helper('sz_rma')->__('The value that you entered is not valid.'),
    ...
    public function getShippingItemsGridAction()
    {
        ...
        $response = $this-> _initAction()
                ->getLayout()
                ->getBlock('sz_rma_getshippingitemsgrid')
                ->toHtml()
        ;
    ...
    public function saveShippingAction()
    {
        ...
        try {
            ...
            if ($model) {
                ...
                Mage::logException($e);
                $responseAjax->setError(true);
                $responseAjax->setMessage(Mage::helper('sz_rma')->__('An error occurred while creating shipping label.'));
        }
    ...
    public function createLabelAction()
    {
        ...
        try {
            ...
            if ($this->_createShippingLabel($shipment)) {
                $shipment->save();
                $this->_getSession()->addSuccess(Mage::helper('sz_rma')->__('The shipping label has been created.'));
                $response->setOk(true);
            }
        } catch (Mage_Core_Exception $e) {
            ...
            Mage::logException($e);
            $response->setError(true);
            $response->setMessage(Mage::helper('sz_rma')->__('An error occurred while creating shipping label.'));
        }
     ...
     * @return bool
     */
    protected function _createShippingLabel(Sz_Rma_Model_Rma $model)
    {
        $data = $this->getRequest()->getPost();
        if ($model && isset($data['packages']) && !empty($data['packages'])) {
            /** @var $shipment Sz_Rma_Model_Shipping */
            $shipment =  Mage::getModel('sz_rma/shipping')
                ->getShippingLabelByRma($model);

            $carrier = Mage::helper('sz_rma')->getCarrier($data['code'], $model->getStoreId());
            ...
            if (!$response->hasErrors() && $response->hasInfo()) {
                ...
                $shipment->setIsAdmin(Sz_Rma_Model_Shipping::IS_ADMIN_STATUS_ADMIN_LABEL);
                ...
                if ($trackingNumbers) {
                    Mage::getResourceModel('sz_rma/shipping')->deleteTrackingNumbers($model);
                    foreach ($trackingNumbers as $trackingNumber) {
                        Mage::getModel('sz_rma/shipping')
                            ->setTrackNumber($trackingNumber)
                            ->setCarrierCode($carrierCode)
                            ->setCarrierTitle($carrierTitle)
                            ->setRmaEntityId($model->getId())
                            ->setIsAdmin(Sz_Rma_Model_Shipping::IS_ADMIN_STATUS_ADMIN_LABEL_TRACKING_NUMBER)
                            ->save();
                    }
                }
                return true;
    ...
    public function printLabelAction()
    {
        try {
            $model = $this->_initModel();
            $labelContent = Mage::getModel('sz_rma/shipping')
                ->getShippingLabelByRma($model)
                ->getShippingLabel();
            if ($labelContent) {
                $pdfContent = null;
                if (stripos($labelContent, '%PDF-') !== false) {
                    ...
                    if (!$page) {
                        $this->_getSession()->addError(Mage::helper('sz_rma')->__('File extension not known or unsupported type in the following shipment: %s', $model->getIncrementId()));
                    }
                    $pdf->pages[] = $page;
            ...
            Mage::logException($e);
            $this->_getSession()
                ->addError(Mage::helper('sz_rma')->__('An error occurred while creating shipping label.'));
       }
        ...
        if ($shipment) {
            $pdf = Mage::getModel('sales/order_pdf_shipment_packaging')
                    ->setPackageShippingBlock(
                        Mage::getBlockSingleton('sz_rma/adminhtml_rma_edit_tab_general_shippingmethod')
                    )
                    ->getPdf($shipment);
            $this->_prepareDownloadResponse(
        ...
        try {
            ...
            if ($model->getId()) {
                Mage::getModel('sz_rma/shipping')
                    ...
                    ->setRmaEntityId($model->getId())
                    ->setIsAdmin(Sz_Rma_Model_Shipping::IS_ADMIN_STATUS_ADMIN_TRACKING_NUMBER)
                    ->save()
                ;
    ...
    public function removeTrackAction()
    {
        ...
        $track      = Mage::getModel('sz_rma/shipping')->load($trackId);
        if ($track->getId()) {
            try {
                $model = $this->_initModel();
                if ($model->getId()) {
