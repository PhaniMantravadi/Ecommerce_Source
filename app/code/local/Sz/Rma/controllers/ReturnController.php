...
class Sz_Rma_ReturnController extends Mage_Core_Controller_Front_Action
{
    /**
     * Action predispatch
     *
    ...
    public function historyAction()
    {
        ...
        $this->getLayout()->getBlock('head')->setTitle(Mage::helper('sz_rma')->__('My Returns'));
    ...
    public function createAction()
    {
        ...
        if ($this->_canViewOrder($order)) {
            ...
            if (($post) && !empty($post['items'])) {
                try {
                    $rmaModel = Mage::getModel('sz_rma/rma');
                    $rmaData = array(
                    ...
                    Mage::getSingleton('core/session')->addSuccess(
                        Mage::helper('sz_rma')->__('Return #%s has been submitted successfully', $rmaModel->getIncrementId())
                    );
                    $this->_redirectSuccess(Mage::getUrl('*/*/history'));
                    return;
                } catch (Exception $e) {
                    Mage::getSingleton('core/session')->addError(
                        Mage::helper('sz_rma')->__('Cannot create New Return, try again later')
                    );
                    Mage::logException($e);
                }
            }
            ...
            $this->getLayout()->getBlock('head')->setTitle(Mage::helper('sz_rma')->__('Create New Return'));
            if ($block = $this->getLayout()->getBlock('customer.account.link.back')) {
                $block->setRefererUrl($this->_getRefererUrl());
            }
            $this->renderLayout();
     ...
     *
     * @param   Sz_Rma_Model_Rma | Mage_Sales_Model_Order $item
     * @return  bool
     */
    ...
    {
        if (Mage::helper('sz_rma')->canCreateRma($orderId)) {
            return true;
        }

        $incrementId    = Mage::registry('current_order')->getIncrementId();
        $message        = Mage::helper('sz_rma')->__('Cannot create rma for order #%s.', $incrementId);
    ...
    public function viewAction()
    {
        ...
        $this->getLayout()
            ->getBlock('head')
            ->setTitle(Mage::helper('sz_rma')->__('RMA #%s', Mage::registry('current_rma')->getIncrementId()));
    ...
    public function addLabelAction()
    {
        if ($this->_loadValidRma()) {
            try {
                ...
                if (!$rma->isAvailableForPrintLabel()) {
                    Mage::throwException(Mage::helper('sz_rma')->__('Shipping Labels are not allowed.'));
                }
                ...
                $carriers  = Mage::helper('sz_rma')->getShippingCarriers($rma->getStoreId());

                if (!isset($carriers[$carrier])) {
                    Mage::throwException(Mage::helper('sz_rma')->__('Select valid carrier.'));
                }

                if (empty($number)) {
                    Mage::throwException(Mage::helper('sz_rma')->__('Enter valid Tracking Number.'));
                }
                ...
                $response = array(
                    'error'     => true,
                    'message'   => Mage::helper('sz_rma')->__('Cannot add label.')
                );
            }
        } else {
            $response = array(
                'error'     => true,
                'message'   => Mage::helper('sz_rma')->__('Wrong RMA Selected.')
            );
        }
        if (is_array($response)) {
        ...
        $this->loadLayout();
        $response = $this->getLayout()->getBlock('sz_rma_return_tracking')->toHtml();
    ...
    public function delLabelAction()
    {
        if ($this->_loadValidRma()) {
            try {
                ...
                $trackingNumber = Mage::getModel('sz_rma/shipping')
                    ->load($number);
                if ($trackingNumber->getRmaEntityId() !== $rma->getId()) {
                    Mage::throwException(Mage::helper('sz_rma')->__('Wrong RMA Selected.'));
                }
                ...
                $response = array(
                    'error'     => true,
                    'message'   => Mage::helper('sz_rma')->__('Cannot delete label.')
                );
            }
        } else {
            $response = array(
    ...
    {
        return Mage::helper('sz_rma')->isEnabled();
    }
}
